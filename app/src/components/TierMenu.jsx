...
// Menu admin de choix du rang, ouvert en cliquant le badge EDS d'une carte
// (mode admin uniquement). Positionne pres du badge clique.
export default function TierMenu({ currentTier, pos, onPick, onClose }) {
  const options = TIERS.map((tier) => ({
    ...
    style: {
      ...
      ...(currentTier === tier.key ? { outline: "2px solid #0f1424", outlineOffset: 1 } : {}),
    },
  }));
  if (currentTier) {
    options.push({
  ...
  return (
    <>
      ...
      <div
        ...
        role="menu"
        aria-label="Choisir le rang EDS Limoges"
      >
        {options.map((opt) => (
